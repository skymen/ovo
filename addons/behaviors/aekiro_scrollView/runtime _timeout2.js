...
(function ()
{
	...
	behinstProto.OnTouchStart = function ()
	{
		...
		if(this.scroll.vScrolling){
			this.scroll.prevTouchY = this.proui.Y(this.inst.layer.index)+0.2;
			this.scroll.dy = 0;
			this.scroll.isEnabled = true;
			this.scroll.scrollToX = false;
			this.scroll.scrollToY = false;
		...
		if(this.scroll.hScrolling){
			this.scroll.prevTouchX = this.proui.X(this.inst.layer.index)+0.2;
			this.scroll.dx = 0;
			this.scroll.isEnabled = true;
			this.scroll.scrollToX = false;
			this.scroll.scrollToY = false;
	...
	{
		var self = this;
		this.scroll.stopInertiaTimeout = setTimeout(function(){self.stopInertia();}, 2000);
	};
	...
	behinstProto.tick = function ()
	{
		...
		if(!this.onSliderTouchStarted){
			...
			if(this.onWheelStarted && this.inertia && Math.abs(this.scroll.dx)<0.1 && Math.abs(this.scroll.dy)<0.1 && !this.scroll.scrollToX && !this.scroll.scrollToY ){
				this.scroll.isEnabled = false;
				this.onWheelStarted = false;
				console.log("Scroll disabled");
			}
